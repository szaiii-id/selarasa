...
const mockShift: MasterShift = {
  id: 1, name: 'Morning Shift', start_time: '08:00:00', end_time: '16:00:00',
  is_active: true, created_at: '2024-01-01T00:00:00+00:00', updated_at: '2024-01-01T00:00:00+00:00'
};

const createWrapper = (props = {}) => {
  return mount(MasterShiftFormModal, {
    props: { isOpen: false, isLoading: false, shiftToEdit: null, errors: {}, ...props }
  });
};
...
describe('MasterShiftFormModal Component', () => {
  describe('Happy & Negative Path', () => {
    ...
    it('[Happy Path] Emit "submit" dengan payload yang benar saat form disubmit (Create Mode)', async () => {
      ...
      expect(emitted?.[0]?.[0]).toEqual({
        name: 'Night Shift', start_time: '00:00', end_time: '08:00', is_active: true
      });
    });
    ...
    it('[Happy Path] Emit "submit" dengan data yang benar saat Edit Mode', async () => {
        ...
        expect(emitted).toBeTruthy();
        expect(emitted?.[0]?.[0]).toEqual({
            name: 'Morning Shift', start_time: '08:00', end_time: '16:00', is_active: true
        });
    });
  ...
  describe('Equivalence Partitioning', () => {
    it('[Partisi 1 - Create Mode] Form di-reset', async () => {
    ...
    it('[Partisi 2 - Edit Mode] Form diisi dengan data', async () => {
      const wrapper = createWrapper({ isOpen: false, shiftToEdit: mockShift });
      await wrapper.setProps({ isOpen: true });
      await wrapper.vm.$nextTick();
      await wrapper.vm.$nextTick();
    ...
    it('[Partisi 5 - Error Display] Menampilkan error message untuk field name', () => {
      const wrapper = createWrapper({ isOpen: true, shiftToEdit: null, errors: { name: ['The name has already been taken.'] } });
    ...
    it('[Partisi 6 - Error Display] Menampilkan error message untuk start_time', () => {
      const wrapper = createWrapper({ isOpen: true, shiftToEdit: null, errors: { start_time: ['Start time is required.'] } });
  ...
  describe('Edge Cases & Corner Cases', () => {
    ...
    it('[Edge Case] Errors dengan multiple field menampilkan semua error', () => {
      const wrapper = createWrapper({
        isOpen: true,
        errors: { name: ['Name is required.'], start_time: ['Start time is required.'], end_time: ['End time is required.'] }
      });
