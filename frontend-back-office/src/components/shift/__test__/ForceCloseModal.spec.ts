...
const mockOpenShift: CashierShift = {
  id: 1, user_id: 'uuid-1', shift_id: 1, opening_balance: 500000,
  closing_balance: null, expected_balance: 1250000, variance: null,
  status: 'open', notes: null, started_at: '2024-01-15T08:00:00+00:00', ended_at: null,
  user: { id: 'uuid-1', name: 'John Doe', username: 'johndoe' },
  shift: { id: 1, name: 'Morning Shift', start_time: '08:00:00', end_time: '16:00:00', is_active: true, created_at: '2024-01-01T00:00:00+00:00', updated_at: '2024-01-01T00:00:00+00:00' }
};

const createWrapper = (props = {}) => {
  return mount(ForceCloseModal, {
    props: { isOpen: false, isLoading: false, shift: null, errors: {}, ...props }
  });
};
...
describe('ForceCloseModal Component', () => {
  ...
  describe('Equivalence Partitioning', () => {
    ...
    it('[Partisi 3 - Notes Kosong] Notes di-reset ke string kosong', async () => {
      const wrapper = createWrapper({ isOpen: false, shift: mockOpenShift });
      await wrapper.setProps({ isOpen: true });
      await wrapper.vm.$nextTick();
      await wrapper.vm.$nextTick();
    ...
    it('[Partisi 4 - Error Display] Menampilkan error untuk expected_balance', () => {
      const wrapper = createWrapper({ isOpen: true, shift: mockOpenShift, errors: { expected_balance: ['Expected balance is required.'] } });
      expect(wrapper.text()).toContain('Expected balance is required.');
    });

    it('[Partisi 5 - Error Display] Menampilkan error untuk notes', () => {
      const wrapper = createWrapper({ isOpen: true, shift: mockOpenShift, errors: { notes: ['Please provide a reason.'] } });
  ...
  describe('Boundary Value Analysis (BVA)', () => {
    ...
    it('[BVA - Button Disabled] Submit button disabled saat isLoading=true', () => {
      const wrapper = createWrapper({ isOpen: true, shift: mockOpenShift, isLoading: true });
      const submitButton = wrapper.findAll('button').find(btn => btn.text().includes('Processing...') || btn.text().includes('Force Close Shift'));
