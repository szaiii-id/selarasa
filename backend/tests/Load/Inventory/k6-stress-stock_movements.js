...
// =========================================================================
// Skenario: Naikkan VUs bertahap sampai sistem mencapai breaking point.
// Threshold longgar karena tujuannya mencari batas sistem.
export const options = {
    scenarios: {
        stress_stock_movements: {
            executor: 'ramping-vus',
            startVUs: 0,
            stages: [
                { duration: '30s', target: 50 },    // Warm-up
                { duration: '1m',  target: 100 },   // Baseline load
                { duration: '1m',  target: 200 },   // Stress 1
                { duration: '1m',  target: 400 },   // Stress 2
                { duration: '1m',  target: 800 },   // Stress 3 — break point area
                { duration: '1m',  target: 800 },   // Hold at break point
    ...
    thresholds: {
        // Threshold longgar karena stress test mencari batas sistem
        'csrf_success_rate': ['rate>0.90'],
        'login_success_rate': ['rate>0.85'],

        'http_req_duration{type:stock_in}': ['p(95)<3000'],
        'stock_in_success': ['rate>0.80'],

        'http_req_duration{type:stock_out}': ['p(95)<3000'],
        'stock_out_success': ['rate>0.75'],

        'http_req_duration{type:get_movements}': ['p(95)<2000'],
        'get_movements_success': ['rate>0.80'],

        // Toleransi lebih besar karena stress test
...
export default function () {
    ...
    // STRESS: Full cycle IN → OUT → GET → ADJUSTMENT
    // Setiap iterasi melakukan 4 operasi untuk memperbesar beban

    // 1. STOCK IN
    group('Stress: Stock IN', function () {
        const uniqueRef = `STRESS-IN-${__VU}-${__ITER}-${Date.now()}`;
        ...
        if (success) {
            ...
            logError('STRESS_IN', res, user);
        }
    });

    // 2. STOCK OUT
    group('Stress: Stock OUT', function () {
        const uniqueRef = `STRESS-OUT-${__VU}-${__ITER}-${Date.now()}`;
        const payload = JSON.stringify({
            raw_material_id: MATERIAL_ID,
            movement_type: 'OUT',
            quantity: 1,
            reason: 'K6 stress OUT',
            reference_id: uniqueRef,
        });
    ...
    // 3. ADJUSTMENT
    group('Stress: Adjustment', function () {
        const uniqueRef = `STRESS-ADJ-${__VU}-${__ITER}-${Date.now()}`;
        const payload = JSON.stringify({
            raw_material_id: MATERIAL_ID,
            movement_type: 'ADJUSTMENT',
            quantity: 1,
            reason: 'K6 stress adjustment',
            reference_id: uniqueRef,
        });

        const start = Date.now();
        const res = http.post(`${BASE_URL}/api-test/v1/backoffice/inventory/movements`, payload, {
            headers: authHeaders, jar: jar, tags: { type: 'adjustment' },
        });
    ...
    // 4. GET MOVEMENTS
    group('Stress: Get Movements', function () {
        const start = Date.now();
        const res = http.get(
            `${BASE_URL}/api-test/v1/backoffice/inventory/movements?raw_material_id=${MATERIAL_ID}&per_page=20`,
...
export function setup() {
    console.log('=== STRESS Test: Stock Movements ===');
    console.log(`Base URL: ${BASE_URL}`);
    console.log(`Target Material ID: ${MATERIAL_ID}`);
    console.log('Scenario: 0 → 50 → 100 → 200 → 400 → 800 VUs');
    console.log('Finding breaking point of the system...');
    console.log('===================================');
...
export function teardown(data) {
    ...
    console.log(`Server Errors (5xx): ${serverErrors.count}`);
